...
def ProcessRSE(name, type):
    ...
    if (type == 1):  # minor signal indicator
        ndx_i = 9    # index of indicator for Major signal
    ...
    # Open input file
    #
    filename = "c:/temp/"+name+"_sorted_.csv"
...
# ** Process the Raw .csv file
    ...
    outfile.close()
# Mutate
    #datalist = MT.ShortenRedMaj(datalist1)
#
...
#
    alldata = []
    alldata = CD.ComputeCycleTime(Start_Time,datalist) # returns a list of tuples: epochtime,cum,red,green,yellow
    cycledata = alldata[0]
    ...
    alldata = CD.ComputeCycleTime(Start_Time,datalist2) # returns a list of tuples: epochtime,cum,red,green,yellow
    cycledata2 = alldata[0]
    errormarks2 = alldata[1]
    # write out scatter plot data for cycles
    outfilename = outfilename = name+"_ScatterNormMutCycle_Data.csv"
    print(" Opening file: ",outfilename)
    try:
        outfile = open(outfilename, 'w')
    except IOError:
...
# generate plot lines from indicators
    lines = BuildMultiIndicatorData(name, datalist, datalist2, datalist3)
# 
